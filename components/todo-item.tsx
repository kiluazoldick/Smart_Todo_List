...
type TodoItemProps = {
  ...
  onUpdate: (id: number, newText: string, newDatetime: Date | null) => void
}
...
export default function TodoItem({ todo, onToggle, onDelete, onUpdate }: TodoItemProps) {
  ...
  return (
    <motion.li
      ...
      className="flex flex-col p-2 bg-card rounded-lg shadow-sm"
    ...
    >
      ...
      {isEditing ? (
        ...
        todo.datetime && (
          <span className="text-xs text-muted-foreground mt-1">{format(todo.datetime, "PPP 'at' HH:mm")}</span>
        )
      )}
    </motion.li>
  )
